...
class DeviceTypeController extends Controller {
    ...
    public function getIndex() {
        ...
        $itemsQ = Device::orderBy( 'company' );
        if ( Input::has( 'company' ) && ! empty( Input::get( 'company' ) ) ) {
//			$itemsQ = $itemsQ->where( 'lower(name)', 'like', strtolower(Input::get( 'name' ) . '%') );
            $itemsQ = $itemsQ->whereRaw('LOWER(name) LIKE ? ',[trim(strtolower(Input::get( 'company' ))).'%']);
            $itemsQ = $itemsQ->orWhere( 'abbreviation', 'like', strtoupper(Input::get( 'company' ) . '%'));
        }
        $items = $itemsQ->paginate( 20 );

        return View::make( 'devices.index', compact( 'items' ) );
    }
    ...
    public function postUpdate( Request $request, $id ) {
        ...
        try {
            ...
            //// Logging
            $logMsg     = $sessionUser->name . " " . $sessionUser->lname . " Updated Device $model->id";
    ...
    public function getDestroy( $id ) {
        ...
        try {
            ...
            $item->delete();
            //// Logging
            $logMsg     = $sessionUser->name . " " . $sessionUser->lname . " Deleted Device $item->id";
            $routeParts = explode( '@', Route::getCurrentRoute()->getActionName() );


        } catch ( QueryException $ex ) {
            if ( $ex->getCode() == 23000 ) {
                Session::flash( 'message', "Can't delete Device because there is assigned types to it." );
            } else {
